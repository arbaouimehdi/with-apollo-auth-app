...
export default async (event: FunctionEvent<EventData>) => {
  ...
  try {
    ...
    const accountActivationCode: string = await createUserAccountActivationCode(
      api,
      id,
    );
    ...
    return {
      error:
        "An unexpected error occured during creation of verificationCode and sending the URL.",
    };
  }
};

async function createUserAccountActivationCode(
  api: GraphQLClient,
  userId: string,
): Promise<string> {
  const mutation = `
  ...
  return api
    .request<{ createAccountActivationCode: AccountActivationCode }>(
      mutation,
      variables,
    )
