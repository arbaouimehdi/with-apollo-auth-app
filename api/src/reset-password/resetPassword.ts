...
export default async (event: FunctionEvent<EventData>) => {
  ...
  try {
    ...
    const passwordResetItem: PasswordResetCode = await getPasswordResetCode(
      api,
      passwordResetCodeId,
    ...
    if (
      !passwordResetItem ||
      !passwordResetItem.id ||
      !passwordResetItem.user
    ) {
      return {
        error: `Password reset not successful 1 ${JSON.stringify(
          passwordResetItem,
        )}`,
      };
    }
    ...
    const userWithNewPassword: UpdatedUser = await setUserPassword(
      api,
      passwordResetItem.user.id,
      newPasswordHash,
    );
